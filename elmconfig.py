...
def convert_conf_item(key: str, val: Any, args: Namespace, parser: ArgumentParser):
    store_actions = {
        act for name, act in parser._registries["action"].items() if isinstance(name, str) and name.startswith("store_")
    }
    for opt in parser._get_optional_actions():
...
class ELMFuzzConfig:
    ...
    def __init__(
        self, prog: str = None, default_config_file: str = "config.yaml", parents: Dict[str, ArgumentParser] = None
    ):
        if prog is None:
    ...
    def init_dumper(self) -> None:
        ...
        self.yaml.representer.add_representer(PosixPath, lambda r, v: r.represent_str(str(v)))

    def _merge_yaml_nodes(self, lhs: Any, rhs: Any) -> Any:
        if lhs is None:
            return copy.deepcopy(rhs)
        if rhs is None:
            return lhs
        if isinstance(lhs, dict) and isinstance(rhs, dict):
            merged = copy.deepcopy(lhs)
            for key, val in rhs.items():
                if key in merged:
                    merged[key] = self._merge_yaml_nodes(merged[key], val)
                else:
                    merged[key] = copy.deepcopy(val)
            return merged
        # For scalars, lists, and type mismatches, later config files override earlier ones.
        return copy.deepcopy(rhs)

    def _merge_yaml_files_fallback(self, files: List[str]) -> CommentedMap:
        merged: Any = CommentedMap()
        for yaml_file in files:
            with open(yaml_file) as fp:
                data = self.yaml.load(fp.read())
            if data is None:
                continue
            merged = self._merge_yaml_nodes(merged, data)
        return merged
    ...
    def merge_yaml_files(self, files: List[str]) -> CommentedMap:
        ...
        for yaml_file in files[::-1]:
            if len(mergers) < 1:
                (mergers, mergers_loaded) = get_doc_mergers(log, yaml_editor, merge_config, yaml_file)
            else:
                try:
                    exit_state = merge_docs(log, yaml_editor, merge_config, mergers, yaml_file)
                except AttributeError as exc:
                    if "yaml_set_tag" in str(exc):
                        # yamlpath may fail on sequence tag updates for some ruamel versions.
                        return self._merge_yaml_files_fallback(files)
                    raise
                if not exit_state == 0:
        ...
        for doc in mergers:
            try:
                ...
                # Some yamlpath/ruamel combinations raise on CommentedSeq tag updates.
                if "yaml_set_tag" not in str(exc):
                    raise
    ...
    def init_parser(self) -> ArgumentParser:
        ...
        config_group.add_argument(
            "--config", default=None, type=Path, help="Path to config file (overrides default search)"
        )
        ...
        group.add_argument("--target.srcs", type=Path, nargs="+", action="extend", help="Source files in the target")
        group.add_argument("--target.covbin", type=Path, help="Path to the target binary with coverage instrumentation")
        ...
        group.add_argument(
            ...
            help="List of model endpoints, formatted as name:endpoint",
        )
        ...
        group.add_argument("--run.num_selected", type=int, default=10, help="Number of seeds to select each generation")
        group.add_argument(
            "--run.genvariant_dir",
            type=str,
        ...
        group.add_argument(
            "--run.logdir", type=str, default="{ELMFUZZ_RUNDIR}/{GEN}/logs", help="Directory (template) to store logs"
        )
        group.add_argument("--run.clean", action="store_true", help="Clean the output directories before running")
    ...
    def config_file_search(self) -> List[str]:
        ...
        config_files.append(os.path.join(script_dir, self.default_config_file))
        # Check CWD
        config_files.append(self.default_config_file)
        # Check ELMFUZZ_RUNDIR env var
        if "ELMFUZZ_RUNDIR" in os.environ:
            config_files.append(os.path.join(os.environ["ELMFUZZ_RUNDIR"], self.default_config_file))
    ...
    def unflatten_conf(
        self, conf: Dict[str, List[Tuple[str, Any, str]]], header_comment: bool = True, base_indent: int = 0, **kwargs
    ) -> CommentedMap:
        ...
        if "cli" in conf:
            ...
            for cli_name, cli_conf in conf["cli"].items():
                ...
                cli_unflattened = self.unflatten_conf(
                    {cli_name: cli_conf}, header_comment=False, base_indent=map_indent * 2, **kwargs
                )
            ...
            if cli_section:
                ...
                if not kwargs.get("skip_comments", False):
                    ...
                    cli_header = "\n" + cli_header + "\n\n"
                    nested_dict.yaml_set_comment_before_after_key("cli", before=cli_header)
    ...
    def get_config(
        ...
        utility_opt_names = {
            opt.dest for parent_parser in self.parent_parsers.values() for opt in parent_parser._get_optional_actions()
        }
...
def get_config_for_progs(progs: List[str], **kwargs) -> CommentedMap:
    ...
    for prog in progs:
        ...
        config = ELMFuzzConfig(prog=prog, parents={prog: parser})
...
ALL_PROGS = ["genvariants_parallel", "geninputs_group_parallel", "genoutputs", "getcov"]


def defaultconfig_cmd(args):
    options = {k: v for k, v in args.__dict__.items() if k.startswith("skip_")}
    ...
    yaml.representer.add_representer(PosixPath, lambda r, v: r.represent_str(str(v)))
...
def main():
    ...
    parser.add_argument(
        "-p", "--prog", type=str, action="append", dest="progs", help="Select programs", choices=ALL_PROGS, default=None
    )
    subparsers = parser.add_subparsers(dest="subcommand", required=True)
    cmd = subparsers.add_parser("dumpconfig", help="Dump config to YAML")
    cmd.add_argument(
        "--skip-comments", action="store_true", default=argparse.SUPPRESS, help="Skip adding help text as comments"
    )
    cmd.add_argument(
        "--skip-defaults",
        action="store_true",
    ...
    cmd.add_argument("--skip-names", type=str, nargs="+", default=argparse.SUPPRESS, help="Names of options to skip")
    cmd.add_argument(
        "--file", type=argparse.FileType("w"), default=sys.stdout, help="File to dump to (default: stdout)"
    )
