...
if [ "$prev_gen" == "initial" ]; then
    echo "First generation; using seed(s):" "$ELMFUZZ_RUNDIR"/initial/seeds/*
    cp -r "$ELMFUZZ_RUNDIR"/initial/seeds/* "$ELMFUZZ_RUNDIR"/${next_gen}/seeds/
else
    # Selection
    ...
    if [ "$selection_strategy" == "lattice" ]; then
        ...
        #         cp -r "$ELMFUZZ_RUNDIR"/${gen}/outputs/${model}/${generator} \
        #         "$ELMFUZZ_RUNDIR"/${next_gen}/seeds/${gen}_${model}_${generator}
        #     done
    ...
    seed_num="$(find "${ELMFUZZ_RUNDIR}/${next_gen}/seeds" -maxdepth 1 -mindepth 1 -type d -printf x | wc -c)"
fi
...
for model_name in $MODELS ; do
    ...
    python geninputs_group_parallel.py $VARIANT_ARGS \
        --backend "$LLM_BACKEND" -M "${model_name}" -O "$GOOUT" -L "$GVLOG" \
