...
def _is_seed_input(name: str) -> bool:
    """True for real seed input files (skip provenance / placeholder files).

    Mirrors ``direct_initial._is_corpus_input`` so a folder that only holds a
    placeholder README / .gitkeep counts as empty here too.
    """
    return name not in ("SOURCE.txt", "README.md", "README") and not name.startswith(".")


def _resolve_seed_corpus(benchmark: str) -> str:
    """Locate the directory of LLM-pregenerated initial inputs for this benchmark.

    Direct mode's initial population is a batch of inputs that were themselves
    generated by an LLM (see ``llm_seed_inputs/{sut}/``); these replace the older
    web-fetched online corpora so direct mode gets no real-world coverage head
    start over synth mode.

    Priority: the ELFUZZ_DIRECT_SEED_CORPUS env override, then
    ``llm_seed_inputs/{benchmark}/``. Returns "" if neither yields any real input
    files (a folder with only a placeholder README/.gitkeep counts as empty).
    """
    env = os.environ.get("ELFUZZ_DIRECT_SEED_CORPUS", "").strip()
    if env:
        return env
    d = os.path.join(PROJECT_ROOT, "llm_seed_inputs", benchmark)
    try:
        if os.path.isdir(d) and any(
            _is_seed_input(f) and os.path.isfile(os.path.join(d, f))
...
def synthesize_direct(
    ...
    *,
    ...
    if not seed_corpus:
        raise click.ClickException(
            f"No LLM-pregenerated inputs for {benchmark!r}: add files to "
            f"llm_seed_inputs/{benchmark}/ (or set ELFUZZ_DIRECT_SEED_CORPUS)."
        )
    click.echo(f"Using LLM-pregenerated initial inputs: {seed_corpus}")
