...
def setup():
    ...
    cmd = ["/home/appuser/elmfuzz/.devcontainer/setup_docker.sh"]
...
@cli.command(
    name="synth",
    help="Synthesize input generators by ELFuzz and its four variants, mine grammars by GLADE, or learn semantic constraints by ISLearn.",
)
@click.option(
    "--target",
    "-T",
...
def config(list_: bool, set: tuple[str, str], get: str):
    assert (
        (list_ is True and set is None and get is None)
        or (list_ is False and set is not None and get is None)
        or (list_ is False and set is None and get is not None)
    ...
    if list_:
        click.echo(
            ...
            |copilot.github_token (default: None): GitHub token for accessing GitHub Copilot.
        """,
                delimiter="\n",
            )
        )
        ...
        match key:
            ...
            case "tgi.huggingface_token":
                token_path = str(HUGGINGFACE_TOKEN_PATH.parent)
                if not os.path.exists(token_path):
                    os.makedirs(token_path, exist_ok=True)
                with open(str(HUGGINGFACE_TOKEN_PATH), "w") as f:
                    f.write(value)
                click.echo(f"{key} := {value}")
                return
            case "copilot.github_token":
                token_path = str(COPILOT_TOKEN_PATH.parent)
                if not os.path.exists(token_path):
                    os.makedirs(token_path, exist_ok=True)
                with open(str(COPILOT_TOKEN_PATH), "w") as f:
                    f.write(value)
                if not os.path.exists("/root/.config/github-copilot"):
                    cmd1 = ["sudo", "mkdir", "-p", "/root/.config/github-copilot"]
                ...
                cmd2 = [
                    ...
                    "-f",
                    "-s",
                    str(COPILOT_TOKEN_PATH),
                    "/root/.config/github-copilot/token",
                ]
        ...
        match get:
            ...
            case "tgi.huggingface_token":
                token_path = str(HUGGINGFACE_TOKEN_PATH)
                if not os.path.exists(token_path):
                    click.echo("None")
                    return
                with open(token_path, "r") as f:
                    value = f.read().strip()
            case "copilot.github_token":
                token_path = str(COPILOT_TOKEN_PATH)
                if not os.path.exists(token_path):
                    click.echo("None")
                    return
                with open(token_path, "r") as f:
...
@click.option(
    "--record-id",
    type=str,
    default=download_mod.DEFAULT_RECORD_ID,
    required=False,
    show_default=True,
)
...
@click.argument(
    "benchmark",
    required=True,
    type=click.Choice(["jsoncpp", "re2", "sqlite3", "cpython3", "libxml2", "librsvg", "cvc5"]),
)
...
@run.command(
    name="rq1.afl",
    help="Run the AFL++ fuzzing compaigns for Figure 8 in RQ1 on BENCHMARKS (represented as a list separated by `,`).",
)
...
@click.option(
    "--time", "-t", type=int, default=86400, show_default=True, help="Time limit for the fuzzing campaign in seconds."
)
@click.option(
    "--parallel",
    "-j",
...
@click.option("--resume", "-R", is_flag=True, default=False, help="Resume a previous fuzzing campaign.")
...
@click.option("--all", "-a", is_flag=True, default=False, help="Reproduce all figures and tables.")
def plot(all: bool):
    ...
    time_cost_cmd = ["python", os.path.join(PROJECT_ROOT, "analysis", "timecost", "time_unit_convert.py")]
    subprocess.run(time_cost_cmd, check=True)
    shutil.copy(
        os.path.join(PROJECT_ROOT, "analysis", "timecost", "x_record_second.csv"),
        os.path.join(PLOT_DATA_DIR, "x_record_second.csv"),
    )

    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq1", "results", "seed_cov.xlsx"),
        os.path.join(PLOT_DATA_DIR, "seed_cov.xlsx"),
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq1", "results", "rq1_sum.xlsx"), os.path.join(PLOT_DATA_DIR, "rq1_sum.xlsx")
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq1", "results", "rq1_std.xlsx"), os.path.join(PLOT_DATA_DIR, "rq1_std.xlsx")
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq2", "results", "rq2_count_bug.xlsx"),
        os.path.join(PLOT_DATA_DIR, "rq2_count_bug.xlsx"),
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq2", "results", "rq2_std.xlsx"), os.path.join(PLOT_DATA_DIR, "rq2_std.xlsx")
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq2", "results", "rq2_time_to_trigger.xlsx"),
        os.path.join(PLOT_DATA_DIR, "rq2_time_to_trigger.xlsx"),
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq2", "results", "unique.xlsx"), os.path.join(PLOT_DATA_DIR, "unique.xlsx")
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq3", "results", "rq3_ablation.xlsx"),
        os.path.join(PLOT_DATA_DIR, "rq3_ablation.xlsx"),
    )
    shutil.copy(
        os.path.join(ANALYSIS_RESULT_DIR, "rq3", "results", "rq3_evolve_cov.xlsx"),
        os.path.join(PLOT_DATA_DIR, "rq3_evolve_cov.xlsx"),
    )
