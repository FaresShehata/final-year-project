...
\begin{enumerate}
  ...
  \item Triage, deduplicate (e.g., by coverage/signature), and minimise crashing inputs for efficient regression testing.
\end{enumerate}
AFL++ extends this loop with numerous engineering improvements (e.g., richer mutation operators and scheduling heuristics, dictionaries, context-sensitive coverage, and persistent execution modes) \cite{fioraldi_2020_aflplusplus} while retaining the same core principle: lightweight feedback steers mutations toward inputs that explore novel paths and uncover deeper bugs \cite{bhme_2016_coveragebased}.

While highly effective, mutation-based fuzzing is sensitive to seed quality and diversity: a narrow seed set can trap the search in local minima, whereas diverse seeds improve parser pass rates and enable deeper exploration. Consequently, corpus management (seed curation, minimisation/deduplication, and power scheduling) is a first-class concern in production fuzzers \cite{bhme_2016_coveragebased,serebryany_2016_libfuzzer}.
...
\begin{itemize}
    ...
    \item \textbf{MemorySanitizer (MSan)} detects uses of uninitialised memory \cite{llvmproject_2015_memorysanitizer}.
...
Collectively, coverage and sanitizers serve both as guidance signals and as oracles of ``interestingness,'' enabling fuzzers to triage and minimise crashing or sanitizer-violating inputs \cite{bhme_2016_coveragebased,serebryany_2016_libfuzzer}.
...
ELFuzz proposes a new method of LLM-based fuzzing: instead of directly generating test inputs, it uses an LLM to synthesise generation-based fuzzers themselves, and then evolves those fuzzers using a coverage-guided notion of ``fuzzer space'', until they become strong seed generators for downstream mutation fuzzers (e.g., AFL++). This decomposition turns the hard problem of writing a good generator into many small, LLM-friendly code edits with quantitative feedback at each step \cite{chen_2025_elfuzz}.
...
To keep the population small and diverse, ELFuzz selects a fixed number $(N)$ of elites whose union of cover sets is maximised. Let $V$ be a pool of fuzzers, and $E \subset V$ be the subset which maximises the union of cover sets. $\mathcal{P}_{N}(V)$ represents all $N$-element subsets of $V$. Thus, we have:
...
Subjects under test (seven real-world SUTs, up to $\sim$1.79 MLoC) include structured formats and interpreters: jsoncpp (JSON), libxml2 (XML), re2 (RegEx), CPython (Python), cvc5 (SMT-LIB2), SQLite (SQL), and librsvg (SVG). Baselines include grammar-based and constraint-based input generators (e.g., GLADE/ANTLR pipelines and ISLA constraints) translated or adapted as needed for fair comparison.
...
\begin{itemize}
    \item The prototype initialises from very simple random-byte seed generators for generality. Using stronger initial templates (or LLM-discovered protocol/grammar hints) could further improve convergence.
    \item If an SUT's input language is poorly represented in the LLM's training data (e.g., proprietary/binary formats), the LLM's edits may lack the priors needed to synthesise good fuzzers. Prompt engineering or fine-tuning may be required, and purely binary formats remain challenging.
    \item  ELFuzz currently optimises the input-generation stage that seeds AFL++. Maintaining diversity continually throughout the entire mutation campaign (dynamic co-evolution of seeds) is not explored and is a direction for future work.
    \item On some targets, ELFuzz spends tens of minutes synthesising seeds before the AFL++ phase; while seeds speed up bug discovery thereafter, the up-front cost can matter under tight time budgets.
...
\begin{itemize}
    \item General fuzzing: The distilled prompt summarises the overall input language (e.g., GCC C docs, CVC5 SMT-LIB overview). The loop explores broadly, maintaining diversity by alternating strategies and continuously refreshing the example.

    \item Targeted fuzzing: The distilled prompt focuses on a specific feature (e.g., C keywords typedef/union, C++ std::expected, Go atomic/built-in libraries, Java keywords such as instanceof/synchronized/finally). This improves the ``hit rate'' on the target feature while still discovering off-target behaviours.
\end{itemize}
...
\begin{itemize}
    ...
    \item The generation LLM (StarCoder in the study) is inference-bound, and fuzzing campaigns are bottlenecked by GPU resources, limiting throughput. Also, the autoprompting overhead (on average $\sim$3 minutes per campaign) is modest, but adds baseline time.
...
LMFuzz's code validity is roughly twice that of Fuzz4All across five projects (e.g., C/C++ validity in the 60-70\% range for LMFuzz vs. $\sim$30-37\% for Fuzz4All), though still below specialised, traditional generators with strict grammars.
...
\begin{itemize}
    ...
    \item Performance depends on initial prompt quality and model priors; the scoring function helps, but prompts that omit critical headers/imports or feature details degrade validity (Chapter 4.2.3). Updates to foundation models pose external validity threats.
...
\begin{itemize}
    ...
    \item For new LLM fuzzers, combining adaptive operator selection, prompt-quality scoring, and on-the-fly repair appears crucial to escape the low-validity trap while retaining diversity; hybridising with selective feedback signals (e.g., simple coverage counters) could further deepen exploration.
\end{itemize}
