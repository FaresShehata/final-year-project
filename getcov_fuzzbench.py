...
def main(image: str, input: str, persist: bool, covfile: str, parallel_num: int):
    ...
    with tempfile.TemporaryDirectory(prefix=prefix) as tmpdir:
        ...
        # Preserve the original generated outputs: copy instead of move.
        # Use copytree for directories and copy2 for single files.
        if os.path.isdir(input):
            shutil.copytree(input, target_dir)
        else:
            os.makedirs(target_dir, exist_ok=True)
            shutil.copy2(input, target_dir)
        if access_info is None:
            ...
            cmd.extend(
                [
                    ...
                    f"/usr/bin/bash",
                    "-c",
                    f'python3 /src/elm_getcov_inside_docker.py --input /tmp/input --output /tmp/cov -j {parallel_num} --prog="{covbin_str}" --real-feedback {real_feedback} --afl-timeout={afl_timeout}',
                ]
            )
        else:
            cmd = [
