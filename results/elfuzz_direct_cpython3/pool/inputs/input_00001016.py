...
def _test() -> None:
    ...
    print({chr(i): chr(i).upper() for i in range(97, 123)})
    print({i: i ** 2 if i % 2 == 0 else i // 2 for i in range(10)})
    print({i: i ** 2 if i % 2 == 0 else i // 2 for i in range(10) if i != 5})
    print({i: (lambda x: x**2)(i) for i in range(10)})
    print({i: (pow(i, 2) if i % 2 == 0 else pow(i, 0.5))(i) for i in range(10)})
    print({k: v for k, v in zip((1, 2, 3), (4, 5, 6))})
    print({str(k): v for k, v in enumerate(reversed(range(10)))})

    print()

    print(*map(lambda x: x.lower(), ["A", "B"]))
    print(*enumerate(["A", "B"], start=1))
    print(*zip(*[["A", "B"], [True]]))
    print(*sorted(["A", "C", "B"]))
    print(*reversed("AB"))
    ...
    print(*filter(None, [None, True, False]))
    print(*filter(str.isalpha, map(chr, range(32, 126))))
    print(*filter(bool, []))
    ...
    for x in sorted(iter(sorted), reverse=True):
        print(x)
