...
class ConcatIterator(Generic[T]):
    def __init__(self, *iterables: Iterable[T], reverse=False) -> None:
        self.iterables = iter(iterables)
        self.reverse = reverse

    def __next__(self) -> T:
        try:
            iters = [iter(it)] for it in self.iterables
        except StopIteration:
            raise StopIteration()
        if self.reverse:
            iters[-1].reverse()
        for i, it in enumerate(reversed(iters), start=1):
            try:
                return next(it)
            except StopIteration:
                del iters[i-1]
...
SumIter = Union[int, float]

def sum_iter(a: SumIter, b: SumIter, /) -> SumIter:
    return a + b
...
class Mapper(Protocol[N]):
    def map(self, value: N) -> R: ...

class Reducer(Protocol[R]):
    def reduce(self,        "locals":   {k: repr(v) for k, v in frame.f_locals.items()},
    }
