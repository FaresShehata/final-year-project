...
def unpack_user_record(raw: bytes) -> UserRecord:
    ...
    for field_idx in range(nfields):
        ...
        (field_type, field_tag) = struct.unpack_from(fmt, raw, offset)
        offset += size
        if field_type == 0:
            continue
        elif field_type == 1:
            value = field_tag.decode().rstrip("\0").strip("\"'")               # string
        elif field_type == 2:
            value = int.from_bytes(field_tag, byteorder="big")
        elif field_type == 3:
            value = float.from_bytes(field_tag, byteorder="big")
        else:
            raise ValueError(f"Unknown field type {field_type}")

        result[field_idx] = value
    assert offset == len(raw), "Unexpected trailing data"
    ...
    fields.extend((f"{i}", int) for i in range(nfields - 5))
...
def _test_c_types():
    class MyInt32(ctypes.Structure):
        _fields_ = [("val", ctypes.c_int)]

    class MyStruct(ctypes.Structure):
        _fields_ = [("a", ctypes.c_char), ("b", ctypes.c_uint)]
...
from typing import (
<|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|><|fim_pad|>    Any,
    Callable,
    ClassVar,
    Final,
    Generic,
