...
class LoggingService:
    ...
    def fail_hard(self) -> None:
        raise Exception('Hard fail')


class Mailer(LoggingService):
    def send_email(
        self,
        recipient: str,
        subject:   str,
...
class Logger(LoggingService):
    ...
    def send_mail(self) -> None:
        for line in self._format_text(textwrap.dedent("""\
            Dear Alice,

            You have won £100!

            Sincerely,
            The Board""")).splitlines():
            super().send_email("alice@example.com", "Your guess is...", line)


class LoggerWithLogging(Mailer, Logger):
    pass


# ── Decorators ───────────────────────────────────────────────────────────────

def timer(func: Callable[P, T]) -> Callable[P, T]:  # noqa: E731
