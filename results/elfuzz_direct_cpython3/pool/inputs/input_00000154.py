...
class Circle(Shape):
    ...
    radius: float = TypedDescriptor(float, 0 < ...)
...
class VowelIterator:
    """
    Generator yielding all vowels in the alphabet.
    """
...
class ConsonantGenerator:
    """
    Generator iterates over consonants in the English language.

    The generator yields one letter per invocation.
    """

    def __iter__(self):
        for c in "bcdfghjklmnopqrstuvwxyz":
            yield c


class Punctuation:
    """
    A class whose objects can only be instantiated as literal strings.
    """

    def __init__(self, char: str) -> None:
        if len(char) != 1 or not char.isalpha():
            raise TypeError(f"'char' must be a single alphabetic character")
