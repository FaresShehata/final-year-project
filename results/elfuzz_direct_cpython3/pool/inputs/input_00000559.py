...
class PropertyCache(type):
    """Metaclass that stores descriptor values as properties on the class."""

    def __new__(cls, name, bases, attrs):
        attr_names = set(getattr(base, "_cache_attrs", []) for base in bases)
        attr_names.add(name)
        attrs["_cache_attrs"] = tuple(attr_names)
        cls_obj = super().__new__(cls, name, bases, attrs)
        initialize_properties(cls_obj)
        return cls_obj


def initialize_properties(cls_obj: type):
    """Initialize properties on a class."""
    for attr_name in cls_obj._cache_attrs:
        attr = getattr(cls_obj, attr_name)
        if not isinstance(attr, CachedProperty):
            continue
        prop_attrs = {"fget": attr.__get__, "doc": attr.__doc__}
        setattr(cls_obj, attr_name, property(**prop_attrs))


class Meta(PropertyCache, type):
    """Metaclass that supports descriptor caching."""

    def __init__(cls, *args, **kwargs):
        super().__init__(*args, **kwargs)
        initialize_properties(cls)
