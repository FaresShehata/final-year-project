...
def _test() -> None:
    ...
    s = "this is a test"
    m = re.match("(?P<word>\S+)", s)
    if m is not None:
        print(m.groupdict())

    print()

    print(match("[a-z]+", "aaa").group())

    print()

    print(findall("[a-z]+", "aaa"))

    print()

    print(sub("[a-z]", "_", "aaa.bbb.ccc.ddd"))
    ...
    print(date.today(), datetime.now(), datetime.utcnow(),
            datetime.fromtimestamp(time.time()), sep="\n")

    print()

    print(time.gmtime(1199139791.2243489),
            time.gmtime(1199139791.2243489)[::-1])

    print()

    print(datetime.strptime("2020-08-09T10:11:58",
                            "%Y-%m-%dT%H:%M:%S"),
    ...
    t = Template("${num} bottles of beer on the wall")
    print(t.substitute(num=5))

    print()

    print(Template("$var").safe_substitute(var=1))

    print()

    print(Template("$var").substitute({"var": 1}))
