...
def product(*xss: Iterable[T]) -> Iterator[tuple[T]]:
    iters = [iter(xs) for xs in xss]
    while True:
...
def decorator(fn: Callable[..., Any], *, kwarg: Optional[Any]=None) -> Callable[..., Any]:

    @functools.wraps(fn)
    def decorated_fn(*args, **kwargs):
        kwargs.setdefault("kwarg", kwarg)
        fn(*args, **kwargsfrom collections import Counter, defaultdict, deque
from typing import (
    TYPE_CHECKING,
    Awaitable,
    Callable,
