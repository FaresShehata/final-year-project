...
class ConcreteClassB(BaseClassB):
    a_attr: int = 69
    b_attr: float = 3.14159
    c_attr: complex = 1j - 1j
    d_attr: str = "bar"


class BaseClassC(Dtype):

    @property
    def e_attr(self) -> bool:
        return True


class DerivedClass(C1, C2, C3):
    ...
    @classmethod
    def get_instance(cls, a: int, b: int, c: int) -> DerivedClass:
        return cls(a, b, c=c)


class SingletonMeta(type):
    instances: WeakKeyDictionary[type[Any], Any] = {}
...
class ConcreteClassE(BaseClassD):
...
class ConcreteClassF(AbstractBaseClass):
