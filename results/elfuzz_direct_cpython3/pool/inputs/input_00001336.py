...
class TrampolinedGenerator(Iterator[A]):
    def __init__(self, generator_function: Callable[[Any], Generator]) -> None:
        ...
        self._stack = [self]
        self._has_yields = True

    def __next__(self) -> A:
        while self._has_yields:
            try:
                self._next_yielded_value = next(self._generator)
            except StopIteration:
                raise StopAsyncIteration from None
            except GeneratorExit:
                self._has_yields = False
                raise
            else:
                if isinstance(self._next_yielded_value, TrampolinedGenerator):
                    self._stack.append(self._next_yielded_value)
                    self._next_yielded_value = next(self._next_yielded_value)
                elif self._next_yielded_value is Ellipsis:
                    self._has_yields = False
                    self._next_yielded_value = next(self._stack.pop())
                elif callable(self._next_yielded_value):
                    self._next_yielded_value = call_next_generator(
                        generator=self._next_yielded_value,
                        stack=self._stack
                    )
                else:
                    break

        value = self._next_yielded_value
        self._next_yielded_value = self._stack.pop()._next_yielded_value
        self._has_yields = bool(self._stack)
        return value


def call_next_generator(generator: TrampolinedGenerator, stack: list[TrampolinedGenerator]) -> Any:
    try:
...
# ── Higher-order Functions, Comprehensions & Generators, Coroutines ───────────

get = operator.attrgetter
get_type_hints = get_type_hints
reveal_type = reveal_type
get_running_library_module = get_running_library_module


# ── Itertools ────────────────────────────────────────────────────────────────

itertools_ = itertools_
...
filter_map = itertools.filtersorted
starmap = itertools.starmap
chain = itertools                               get_args, get_type_hints, reveal_type, get_running_library_module)
...
def _test() -> None:
    ...
    print(re.search("^Python", "Python is fun"))
