...
def match(something: str | int) -> str:
    match something:
        ...
        case int():
            return f"integer: {something}"
        case _:
            raise TypeError(f"{type(something).__qualname__} has no known types")


# ── Walrus Operator ───────────────────────────────────────────────────────────

async def get_thing() -> tuple[int, str]:
    await asyncio.sleep(random.random())
    return (random.randint(1, 3), "foo")


async def do_something(thing: tuple[int, str]) -> str:
    i, s = thing  # use the walrus operator
...
eg = ExceptionGroup("group", [ValueError("oops"), RuntimeError("bad!")])
print(eg, eg.exceptions)
eg.raise_from_next()
