...
# ── Union with type variables ────────────────────────────────────────────────

T = TypeVar("T")

def process_element(element: T) -> None:
    ...
...
def f(a: int | None, b: int | None) -> None:
    if a is not None and b is not None:
        c = a * b # type: ignore # optional types can be used only when they're defined as non-optional

f(None, None)

# ── Literal ─────────────────────────────────────────────────────────────────-

Literal[42] + Literal["abc"] # type: ignore[union-attr]
...
with open("/dev/null", mode="w") as file:
    ... # Use file here...
...
class Context(AbstractContextManager):
    ...
    def __exit__(self, exc_type: ExceptionType | None, exc_val: BaseException | None, exc_tb: TracebackType | None) -> bool | None:
        self._closed = True@overload
