...
def run_around_view(array_func: types.FunctionType) -> None:
    ...
    This tests that the underlying structure remains unchanged, even when we modify it.
    """
    data = bytearray([1])
    ...
    assert array_func(arr) == b"\x01\x00\x00\x00"
    assert mview[0] == ord("\x01")

    print("[memoryview]  Before:", repr(mview))
    assert array_func(mview) == b"\x01\x00\x00\x00"
    assert mview[0] == ord("\x01")


def add_bytes_to_array(arr: array.array, offset: int, bytes_: bytes) -> None:
    arr[offset : offset + len(bytes_)] = bytes_
