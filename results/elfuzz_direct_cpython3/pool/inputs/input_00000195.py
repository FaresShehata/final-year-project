...
def debug_prints(level: int = 0):
    ...
    def decorate(print_fn: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            print((("#" * level)[::-1]) + ": " + print_fn.__qualname__)
...
def track_values(level: int = 0):
    """Save all values passed through this function to an internal dict.

    This allows us to test how many times each argument was used, which can be useful
    during debugging or benchmarking to spot unneeded or unnecessary computations.
    """

    def decorate(func: Callable):
        func._values_called = defaultdict(int)
        func._value_references = {}      # TODO: Make this a set?

        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            # Track arguments by position
            index = 1
            for arg in args:
                func._values_called[index] += 1
                func._value_references[index] = id(arg)
                index += 1

            # Track keyword arguments by key
            for key, val in kwargs.items():
                func._values_called[key] += 1
                func._value_references[key] = id(val)
...
def timing_decorator(duration: float = 0.0):
    """
    Apply decorators to a function that will measure execution duration and throw
    an exception if the decorated function takes longer than specified time.
import sys
import textwrap
import tracemalloc
import types
