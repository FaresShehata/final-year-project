...
class SortedList(Generic[T]):
    ...
    def insert(self, index: int, item: T) -> None:
        bisect.insort_left(self.items, item, index=index)
...
class CustomException(Exception):
    ...
    def __str__(self) -> str:
        """Called by print(), str(), or the built-in function `repr()`."""
        return f"{type(self).__qualname__}({', '.join(map(repr,self.args))})"


class ExcGroup(ExceptionGroup):

    def __init__(
        self, first_exception: BaseException, sub_exceptions: Iterable[BaseException]
    ) -> None:
        super().__init__(first_exception, sub_exceptions)
...
def main() -> None:
    ...
    print("\n>>> Walrus Operator ▶️ Special cases")


    def get_input(prompt: str = "") -> int:
        return (i := input(prompt))[::-1][0]
