#!/usr/bin/env python3
"""Pareto-by-superset elite selection for `elfuzz direct` collections.

This is a copy-adapt of `select_seeds.py` with two changes:

1. The fixed MODEL token is ``direct`` (not ``CodeLlama-13b-hf``).
2. The "size" of a unit is the total byte count of all input files in the
   collection directory, instead of the size of one Python file.

Behaviour for the no-proper-elites case is inherited from the original: when
the new generation produces no descendants with usable coverage,
``filtered_descendants0`` ends up empty and ``new_elites`` carries forward the
previous-gen elites. We additionally print a stderr warning so the wrapper
shell script can fall back to copying parents directly when ``new_elites`` is
also empty (i.e. very first iteration produced nothing).
"""
...
def superior_than(edge_coverage1: set[str], edge_coverage2: set[str]) -> bool:
    return len(edge_coverage1) > len(edge_coverage2) and (
        (not edge_coverage2) or edge_coverage2.issubset(edge_coverage1)
    )


def inferior_than(edge_coverage1: set[str], edge_coverage2: set[str]) -> bool:
    return len(edge_coverage1) < len(edge_coverage2) and (
        (not edge_coverage1) or edge_coverage1.issubset(edge_coverage2)
    )
...
def collection_size(collection_dir: str) -> int:
    if not os.path.isdir(collection_dir):
        return 0
    total = 0
    for f in os.listdir(collection_dir):
        p = os.path.join(collection_dir, f)
        if os.path.isfile(p):
            try:
                total += os.path.getsize(p)
            except OSError:
                pass
    # Avoid 0-size (would be a meaningless tiebreak); enforce minimum 1.
    return max(total, 1)
...
def main(
    ...
    if baseline is not None:
        with open(click.format_filename(baseline)) as base_edges_f:
            ...
            for line in base_edges_f:
                if not line.strip():
                    continue
                base_edges.add(line.strip())

    coverage = {
        model: {
            key: set(map(lambda x: x.split(":")[0], val))
            for key, val in cov.items()
        }
        for model, cov in coverage_raw.items()
    }

    if generation == "initial" or generation == "gen0":
        elites: dict[str, tuple[set[str], int]] = dict()
    else:
    ...
    for descendant_key, descendant_edges_raw in coverage_modulo_model.items():
        ...
        descendant_size = collection_size(
            os.path.join(ELMFUZZ_RUNDIR, generation, "variants", MODEL, descendant_key)
        )
    ...
    comparison_raw: dict[
        tuple[str, str], Union[Literal["l"], Literal["r"], Literal["b"]]
    ] = dict()
    ...
    comparison: dict[
        str, dict[str, Union[Literal["l"], Literal["r"], Literal["b"]]]
    ] = dict()
    ...
    if baseline is not None:
        ...
        if interesting:
            print(
                f"Found {len(interesting)} interesting elites with max interesting edges {max_interesting_edges}",
                file=sys.stderr,
            )
    ...
    if baseline is not None and len(interesting) > THRESHOLD_FACTOR * max_elites:
        print(
            f"WARNING: The number of interesting elites {len(interesting)} exceeds the limit {max_elites} x {THRESHOLD_FACTOR}",
            file=sys.stderr,
        )
    ...
    if len(new_elites.items()) > max_elites:
        if len(new_elites) > THRESHOLD_FACTOR * max_elites:
            print(
                f"WARNING: The number of elites {len(new_elites)} exceeds the limit {max_elites} x {THRESHOLD_FACTOR}",
                file=sys.stderr,
            )

            def random_search(
                set_family: list[tuple[str, set[str], int]],
                num: int = max_elites,
                ...
                print(
                    f'Get almost bests: {" ".join(map(lambda item: f"({len(item[0])}, {item[1]})", stat))}',
                    file=sys.stderr,
                )
            ...
            print(
                f"WARNING: The number of elites {len(new_elites)} exceeds the limit {max_elites}",
                file=sys.stderr,
            )
            if baseline is not None:
                interesting_items = []
                ...
                for k, item in new_elites.items():
                    if k in interesting:
                        ...
                        trivial_items.append((k, item))
                sorted_intrested = sorted(
                    interesting_items,
                    key=lambda item: (-len(set(item[1][0]).union(base_edges)), item[1][1]),
                )
                if len(interesting) >= max_elites:
                    new_elites = dict(sorted_intrested[:max_elites])
                else:
                    sorted_trivial = sorted(
                        trivial_items, key=lambda item: (-len(item[1][0]), item[1][1])
                    )
                    new_elites = dict(
                        sorted_intrested + sorted_trivial[: max_elites - len(sorted_intrested)]
                    )
            else:
                new_elites = dict(
                    sorted(new_elites.items(), key=lambda item: (-len(item[1][0]), item[1][1]))[
                        :max_elites
                    ]
                )
    ...
    if not new_elites:
        # Original code's invariant carries the previous-gen elites forward when
        # the new gen produced no usable coverage. If we're at gen0/initial and
        # have nothing yet, signal the wrapper to fall back to the parent seeds.
        print(
            f"WARNING: no proper elites at gen {generation}; carrying forward parents",
            file=sys.stderr,
        )

    output_elite_file.write(json.dumps(new_elites))

    for elite_key, (elite_edges, _) in sorted(
        new_elites.items(), key=lambda item: (len(item[1][0]), -item[1][1])
    ):
        try:
            gen, generator = elite_key.split("-", 1)
        except Exception:
